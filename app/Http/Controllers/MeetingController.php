 ...
 * (indexAsCoach / upsertMemo)を 1 Controller に集約する。Action 化対象の各 method は処理を委譲し、
 * リクエスト受付・認可委譲・レスポンス整形のみを担う。
 */
class MeetingController extends Controller
{
    ...
    {
        return view('meeting.index', $action($request->user(), $request->validated()));
    }
    ...
    {
        return view('meeting.coach.index', $action($request->user(), $request->validated()));
    }
    ...
    public function store(
        ...
        $meeting = $action($enrollment, $request->validated());
    ...
    public function cancel(
        ...
        $meeting = $action($meeting, request()->user());
